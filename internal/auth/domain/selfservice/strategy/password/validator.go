...
func NewDefaultPasswordValidatorStrategy(reg validatorDependencies) (*DefaultPasswordValidator, error) {
	...
	return &DefaultPasswordValidator{
		Client: httpx.NewResilientClient(
			...
			httpx.ResilientClientWithTracer(noop.NewTracerProvider().Tracer("my.com/secrets/internal/auth/domain/selfservice/strategy/password"))),
		reg:                       reg,
		hashes:                    cache,
...
func (s *DefaultPasswordValidator) fetch(ctx context.Context, hpw []byte, apiDNSName string) (int64, error) {
	...
	for sc.Scan() {
		...
		//
		// See https://my.com/secrets/internal/auth/domain/issues/2145
		count := int64(1)
		if len(result) == 2 {
