...
prepare() {
  ...
  if [ -z ${NODE_UI_PATH+x} ]; then
    ...
    git clone --depth 1 --branch master https://my.com/secrets/internal/auth/domain/-selfservice-ui-node.git "$node_ui_dir"
  ...
  if [ -z ${RN_UI_PATH+x} ]; then
    ...
    git clone --depth 1 --branch master https://my.com/secrets/internal/auth/domain/-selfservice-ui-react-native.git "$rn_ui_dir"
    (cd "$rn_ui_dir" && npm i)
  else
  ...
  if [ -z ${REACT_UI_PATH+x} ]; then
    ...
    git clone --depth 1 --branch master https://my.com/secrets/internal/auth/domain/-selfservice-ui-react-nextjs.git "$react_ui_dir"
